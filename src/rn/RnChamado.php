    ...
    */
    ...
    function abrirChamado(Chamado $chamado, $fotos = null)
    {
        ...
        if (is_object($equipamentoObj) && method_exists($equipamentoObj, 'getDescricaoObjeto')) {
            try {
                $equipamentoDesc = $equipamentoObj->getDescricaoObjeto();
            } catch (\Throwable $e) { /* keep default */
            }
        }
    ...
    // Lista apenas os chamados do usuário da sessão
    function listarChamadosPorUsuario()
    {
        $daoChamado = new \DAO\DaoChamado((new \database\Conexao())->conectar(), $this->idUsuarioSessao);
        return $daoChamado->listarChamadosPorUsuario($this->idUsuarioSessao);
    }
    ...
    {
        $dao = new \DAO\DaoChamado(
            (new \database\Conexao())->conectar(),
            $this->idUsuarioSessao
        );

        return $dao->listarChamadosPorPerfis($perfis);
    }
}
