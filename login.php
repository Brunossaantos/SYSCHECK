...
<body class="flex flex-col items-center justify-center min-h-screen text-white p-8">
    ...
    <div class="text-center mb-10">
        <h1 class="text-5xl font-extrabold logo tracking-widest">SYSCHECK</h1>
        <h2 class="text-xl text-gray-300 mt-2">SISTEMA DE CHECKLIST</h2>
    </div>
    ...
    // Se já estiver logado, vai direto para a index
    ...
    <div class="w-full max-w-md animate-fadeIn">
        ...
        <?php if (isset($_SESSION['mensagem_sessao'])): ?>
            <div id="alert-sessao" class="bg-blue-600/20 border border-blue-500 text-blue-100 p-4 rounded-xl mb-6 flex items-center justify-between backdrop-blur-sm">
                <div class="flex items-center">
                    ...
                    <span class="text-sm font-medium"><?= $_SESSION['mensagem_sessao']; ?></span>
                </div>
                <button onclick="this.parentElement.remove()" class="text-blue-300 hover:text-white transition-colors text-2xl leading-none">&times;</button>
            </div>
            <?php unset($_SESSION['mensagem_sessao']); ?>
        <?php endif; ?>

        <div class="bg-gray-800 p-8 rounded-2xl shadow-2xl">
            <h2 class="text-2xl font-bold text-center mb-6 uppercase tracking-wider">LOGIN</h2>
            ...
            <form action="/syscheck/usuario/login" method="POST" class="flex flex-col gap-4">
                ...
                <div class="flex flex-col relative">
                    ...
                    <button type="button" onclick="toggleSenha()" class="absolute right-3 top-10 text-gray-400 hover:text-white">
    ...
    <script>
        ...
        /* Mensagens via URL (Inatividade e Erro) */
        ...
        // Limpa a URL para não repetir o alert no F5
        if (msg || params.get("erro")) {
            window.history.replaceState({}, document.title, window.location.pathname);
        }

        /* Fechar alerta da sessão automaticamente após 6 segundos */
        const alertSessao = document.getElementById('alert-sessao');
        if (alertSessao) {
            setTimeout(() => {
                alertSessao.style.transition = "opacity 0.8s ease";
                alertSessao.style.opacity = "0";
                setTimeout(() => alertSessao.remove(), 800);
            }, 6000);
        }
    </script>
</body>
